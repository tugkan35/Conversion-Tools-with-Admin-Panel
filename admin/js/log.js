const logsData = `

ip=111.7.100.23
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.119 Safari/537.36
referrer=-

ip=111.7.100.22
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.119 Safari/537.36
referrer=http://easyconverter.online/about.html

ip=111.7.100.23
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.119 Safari/537.36
referrer=-

ip=111.7.100.23
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.119 Safari/537.36
referrer=http://easyconverter.online/about.html

ip=111.7.100.22
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.119 Safari/537.36
referrer=http://easyconverter.online/about.html

ip=111.7.100.21
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.119 Safari/537.36
referrer=http://easyconverter.online/about.html

ip=111.7.100.22
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.119 Safari/537.36
referrer=http://easyconverter.online/about.html

ip=111.7.100.21
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.119 Safari/537.36
referrer=http://easyconverter.online/about.html

ip=111.7.100.21
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.119 Safari/537.36
referrer=-

ip=60.13.136.44
browser=Mozilla/5.0 (iPhone; CPU iPhone OS 9_1 like Mac OS X) AppleWebKit/601.1.46 (KHTML, like Gecko) Version/9.0 Mobile Safari/601.1
referrer=-

ip=34.245.9.220
browser=Mozilla/5.0 (Macintosh; Intel Mac OS X 10.15; rv:81.0) Gecko/20100101 Firefox/81.0
referrer=-

ip=52.19.203.139
browser=Mozilla/5.0 (Macintosh; Intel Mac OS X 10.15; rv:81.0) Gecko/20100101 Firefox/81.0
referrer=-

ip=111.7.100.23
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.119 Safari/537.36
referrer=-

ip=111.7.100.22
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.119 Safari/537.36
referrer=http://easyconverter.online/weight.html

ip=111.7.100.21
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.119 Safari/537.36
referrer=http://easyconverter.online/weight.html

ip=111.7.100.23
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.119 Safari/537.36
referrer=-

ip=111.7.100.23
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.119 Safari/537.36
referrer=http://easyconverter.online/weight.html

ip=111.7.100.21
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.119 Safari/537.36
referrer=http://easyconverter.online/weight.html

ip=111.7.100.20
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.119 Safari/537.36
referrer=-

ip=52.42.101.40
browser=Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.97 Safari/537.36
referrer=-

ip=34.213.223.70
browser=Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.97 Safari/537.36
referrer=-

ip=66.249.66.132
browser=Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)
referrer=-

ip=66.249.66.3
browser=Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)
referrer=-

ip=34.253.221.168
browser=Mozilla/5.0 (Macintosh; Intel Mac OS X 10.15; rv:81.0) Gecko/20100101 Firefox/81.0
referrer=-

ip=66.249.66.48
browser=Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)
referrer=-

ip=66.249.64.197
browser=Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)
referrer=-

ip=66.249.64.194
browser=Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)
referrer=-

ip=54.216.112.68
browser=Mozilla/5.0 (Macintosh; Intel Mac OS X 10.15; rv:81.0) Gecko/20100101 Firefox/81.0
referrer=-

ip=40.77.167.50
browser=Mozilla/5.0 (compatible; bingbot/2.0; +http://www.bing.com/bingbot.htm)
referrer=-

ip=40.77.167.38
browser=Mozilla/5.0 (compatible; bingbot/2.0; +http://www.bing.com/bingbot.htm)
referrer=-

ip=62.210.78.106
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:58.0) Gecko/20100101 Firefox/58.0
referrer=-

ip=62.210.78.106
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:58.0) Gecko/20100101 Firefox/58.0
referrer=-

ip=62.210.78.106
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:58.0) Gecko/20100101 Firefox/58.0
referrer=-

ip=62.210.78.106
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:58.0) Gecko/20100101 Firefox/58.0
referrer=-

ip=62.210.78.106
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:58.0) Gecko/20100101 Firefox/58.0
referrer=-

ip=62.210.78.106
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:58.0) Gecko/20100101 Firefox/58.0
referrer=-

ip=62.210.78.106
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:58.0) Gecko/20100101 Firefox/58.0
referrer=-

ip=62.210.78.106
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:58.0) Gecko/20100101 Firefox/58.0
referrer=-

ip=62.210.78.106
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:58.0) Gecko/20100101 Firefox/58.0
referrer=-

ip=62.210.78.106
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:58.0) Gecko/20100101 Firefox/58.0
referrer=-

ip=62.210.78.106
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:58.0) Gecko/20100101 Firefox/58.0
referrer=-

ip=62.210.78.106
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:58.0) Gecko/20100101 Firefox/58.0
referrer=-

ip=62.210.78.106
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:58.0) Gecko/20100101 Firefox/58.0
referrer=-

ip=62.210.78.106
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:58.0) Gecko/20100101 Firefox/58.0
referrer=-

ip=62.210.78.106
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:58.0) Gecko/20100101 Firefox/58.0
referrer=-

ip=62.210.78.106
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:58.0) Gecko/20100101 Firefox/58.0
referrer=-

ip=62.210.78.106
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:58.0) Gecko/20100101 Firefox/58.0
referrer=-

ip=62.210.78.106
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:58.0) Gecko/20100101 Firefox/58.0
referrer=-

ip=34.222.159.109
browser=Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.97 Safari/537.36
referrer=-

ip=34.219.140.99
browser=Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.97 Safari/537.36
referrer=-

ip=34.219.140.99
browser=Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.97 Safari/537.36
referrer=-

ip=34.221.52.193
browser=Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.97 Safari/537.36
referrer=-

ip=54.190.4.236
browser=Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.97 Safari/537.36
referrer=-

ip=34.221.52.193
browser=Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.97 Safari/537.36
referrer=-

ip=34.242.90.92
browser=Mozilla/5.0 (Macintosh; Intel Mac OS X 10.15; rv:81.0) Gecko/20100101 Firefox/81.0
referrer=-

ip=157.55.39.182
browser=Mozilla/5.0 (compatible; bingbot/2.0; +http://www.bing.com/bingbot.htm)
referrer=-

ip=157.55.39.163
browser=Mozilla/5.0 (compatible; bingbot/2.0; +http://www.bing.com/bingbot.htm)
referrer=-

ip=91.227.68.176
browser=Opera/9.80 (Windows NT 6.1; U; MRA 5.9 (build 4953); ru) Presto/2.9.168 Version/11.52
referrer=-

ip=47.243.33.234
browser=Mozilla/5.0 (Macintosh; Intel Mac OS X 10_14_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/76.0.3809.132 Safari/537.36
referrer=-

ip=47.243.33.234
browser=fasthttp
referrer=-

ip=66.249.70.101
browser=Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)
referrer=-

ip=66.249.70.101
browser=Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)
referrer=-

ip=34.219.244.35
browser=Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.97 Safari/537.36
referrer=-

ip=34.219.28.92
browser=Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.97 Safari/537.36
referrer=-

ip=34.219.28.92
browser=Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.97 Safari/537.36
referrer=-

ip=34.216.124.251
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64)AppleWebKit/537.36 (KHTML, like Gecko) Chrome/66.0.3359.181 Safari/537.36
referrer=-

ip=47.242.107.167
browser=Mozilla/5.0 (Macintosh; Intel Mac OS X 10_14_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/76.0.3809.132 Safari/537.36
referrer=-

ip=47.242.107.167
browser=fasthttp
referrer=-

ip=34.124.143.168
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.108 Safari/537.36
referrer=-

ip=34.124.143.168
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.108 Safari/537.36
referrer=-

ip=34.124.143.168
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.108 Safari/537.36
referrer=-

ip=34.124.143.168
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.108 Safari/537.36
referrer=-

ip=34.124.143.168
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.108 Safari/537.36
referrer=-

ip=34.124.143.168
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.108 Safari/537.36
referrer=-

ip=34.124.143.168
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.108 Safari/537.36
referrer=-

ip=34.124.143.168
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.108 Safari/537.36
referrer=-

ip=34.124.143.168
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.108 Safari/537.36
referrer=-

ip=34.124.143.168
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.108 Safari/537.36
referrer=-

ip=34.124.143.168
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.108 Safari/537.36
referrer=-

ip=34.124.143.168
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.108 Safari/537.36
referrer=-

ip=34.124.143.168
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.108 Safari/537.36
referrer=-

ip=34.124.143.168
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.108 Safari/537.36
referrer=-

ip=34.124.143.168
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.108 Safari/537.36
referrer=-

ip=34.124.143.168
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.108 Safari/537.36
referrer=-

ip=34.124.143.168
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.108 Safari/537.36
referrer=-

ip=34.124.143.168
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.108 Safari/537.36
referrer=-

ip=34.124.143.168
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.108 Safari/537.36
referrer=-

ip=34.124.143.168
browser=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.108 Safari/537.36
referrer=-

ip=44.234.67.73
browser=CheckMarkNetwork/1.0 (+http://www.checkmarknetwork.com/spider.html)
referrer=-

ip=44.234.67.73
browser=CheckMarkNetwork/1.0 (+http://www.checkmarknetwork.com/spider.html)
referrer=-

ip=44.234.67.73
browser=CheckMarkNetwork/1.0 (+http://www.checkmarknetwork.com/spider.html)
referrer=-

ip=44.234.67.73
browser=CheckMarkNetwork/1.0 (+http://www.checkmarknetwork.com/spider.html)
referrer=-

ip=44.234.67.73
browser=CheckMarkNetwork/1.0 (+http://www.checkmarknetwork.com/spider.html)
referrer=http://easyconverter.online/

ip=44.234.67.73
browser=CheckMarkNetwork/1.0 (+http://www.checkmarknetwork.com/spider.html)
referrer=http://easyconverter.online/

ip=44.234.67.73
browser=CheckMarkNetwork/1.0 (+http://www.checkmarknetwork.com/spider.html)
referrer=http://easyconverter.online/

ip=44.234.67.73
browser=CheckMarkNetwork/1.0 (+http://www.checkmarknetwork.com/spider.html)
referrer=-

ip=44.234.67.73
browser=CheckMarkNetwork/1.0 (+http://www.checkmarknetwork.com/spider.html)
referrer=-

ip=44.234.67.73
browser=CheckMarkNetwork/1.0 (+http://www.checkmarknetwork.com/spider.html)
referrer=-

ip=44.234.67.73
browser=CheckMarkNetwork/1.0 (+http://www.checkmarknetwork.com/spider.html)
referrer=-

ip=44.234.67.73
browser=CheckMarkNetwork/1.0 (+http://www.checkmarknetwork.com/spider.html)
referrer=-

`;

// Parse logs data into array with sequential IDs starting from 0
let logs = [];
let records = logsData.trim().split('\n\n');
records.forEach((record, index) => {
  let lines = record.trim().split('\n');
  if (lines.length >= 3) {
    let ip = lines[0].replace('ip=', '').trim();
    let browser = lines[1].replace('browser=', '').trim();
    let referrer = lines[2].replace('referrer=', '').trim();
    
    logs.push({
      id: index,
      ip: ip,
      browser: browser,
      referrer: referrer
    });
  }
});

let currentPage = 1;
let perPage = parseInt($('#perPage').val());

function renderTable(){
  $('#totalLogs').text(logs.length);
  let start = (currentPage-1)*perPage;
  let show = logs.slice(start, start+perPage);
  $('#logBody').empty();
  show.forEach(l=>{
    $('#logBody').append(`
      <tr>
        <td>${l.id}</td>
        <td>${l.ip}</td>
        <td>${l.browser}</td>
        <td>${l.referrer}</td>
      </tr>
    `);
  });
  renderPagination();
}

function renderPagination(){
  let totalPages = Math.ceil(logs.length / perPage);
  let pagination = $('#pagination');
  pagination.empty();

  pagination.append(`<li class="page-item ${currentPage===1?'disabled':''}">
    <a class="page-link" href="#" onclick="goPage(${currentPage-1})">Previous</a>
  </li>`);

  let maxVisible = 5;
  let startPage = Math.max(1, currentPage - 2);
  let endPage = Math.min(totalPages, startPage + maxVisible - 1);

  if(startPage > 1){
    pagination.append(pageItem(1));
    if(startPage > 2) pagination.append(ellipsisItem());
  }

  for(let i=startPage; i<=endPage; i++){
    pagination.append(pageItem(i));
  }

  if(endPage < totalPages){
    if(endPage < totalPages - 1) pagination.append(ellipsisItem());
    pagination.append(pageItem(totalPages));
  }

  pagination.append(`<li class="page-item ${currentPage===totalPages?'disabled':''}">
    <a class="page-link" href="#" onclick="goPage(${currentPage+1})">Next</a>
  </li>`);
}

function pageItem(page){
  return `<li class="page-item ${page===currentPage?'active':''}">
    <a class="page-link" href="#" onclick="goPage(${page})">${page}</a>
  </li>`;
}

function ellipsisItem(){
  return `<li class="page-item disabled"><span class="page-link">...</span></li>`;
}

function goPage(p){ currentPage=p; renderTable(); }

$('#perPage').on('change',function(){
  perPage=parseInt($(this).val());
  currentPage=1;
  renderTable();
});

renderTable();